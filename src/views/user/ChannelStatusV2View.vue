        ...
        <div class="min-h-0 max-h-[min(52vh,520px)] overflow-auto p-4 sm:p-5">
          ...
          <div v-else-if="activeTab === 'errors'" class="space-y-3">
            ...
            >
              ...
              <div v-if="expandedErrors.has(row.category)" class="mt-3 border-t border-gray-100 pt-3 dark:border-dark-700">
                <p class="text-xs text-gray-400">{{ t('channelMonitorV2.errorDetail.empty') }}</p>
              </div>
            </div>
          </div>
...
const showUserRanking = computed(() => !isChannelMonitorUserRankingHidden())
...
async function loadDimensions(signal?: AbortSignal, id = sequence) {
  ...
  const next = await api.getDimensions(rangeOnly, signal)
...
async function loadMetrics(signal?: AbortSignal, id = sequence) {
  const [nextSnapshot, nextMatrix] = await Promise.all([
    api.getSnapshot(filter.value, signal),
    api.getMatrix(filter.value, matrixGroupBy.value, signal),
  ])
  if (id !== sequence) return
...
async function loadTab(signal?: AbortSignal, id = sequence) {
  ...
  try {
    if (activeTab.value === 'models') {
      modelRows.value = (await api.getModels(filter.value, signal)).items || []
    } else if (activeTab.value === 'errors') {
      errorRows.value = (await api.getErrors(filter.value, signal)).items || []
    } else if (showUserRanking.value) {
      userRows.value = (await api.getUsers(filter.value, signal)).items || []
    } else {
      userRows.value = []
    }
  } catch (error) {
