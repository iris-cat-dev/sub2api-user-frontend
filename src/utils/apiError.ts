...
/** Translate a metadata field label when a user-facing translation exists. */
...
/** Localize payment metadata fields while preserving unknown backend keys. */
function localizeMetadata(metadata: Record<string, unknown>, t: TranslateFn): Record<string, unknown> {
  const out: Record<string, unknown> = { ...metadata }
  if (typeof out.key === 'string') {
    out.key = tryTranslate(t, `payment.fields.${out.key}`, out.key)
  }
  if (typeof out.keys === 'string') {
    out.keys = out.keys
      .split('/')
      .map(k => tryTranslate(t, `payment.fields.${k}`, k))
      .join(' / ')
  }
  return out
}
 ...
 *
 * Config-field names in metadata (`key` / `keys`) use user-facing labels when
 * available and otherwise remain unchanged.
 *
