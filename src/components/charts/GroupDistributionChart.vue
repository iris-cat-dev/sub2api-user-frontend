<template>
  <div class="card p-4">
    <div class="mb-4 flex items-center justify-between gap-3">
      <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('dashboard.groupDistribution') }}</h3>
    ...
    <div v-else-if="displayItems.length > 0 && chartData" class="flex flex-col items-center gap-4 sm:flex-row sm:gap-6">
      ...
      <div class="max-h-48 w-full min-w-0 flex-1 overflow-auto">
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr v-for="item in displayItems" :key="item.group_id" class="border-t border-gray-100 dark:border-dark-700">
              <td class="max-w-[130px] truncate py-1.5 font-medium text-gray-900 dark:text-white">{{ item.group_name || t('dashboard.noGroup') }}</td>
...
const displayItems = computed(() => {
  ...
  return [...props.groupStats].sort((left, right) => right[metric] - left[metric]).slice(0, 10)
})
const chartData = computed<ChartData<'doughnut'> | null>(() => {
  if (displayItems.value.length === 0) return null
  return {
    labels: displayItems.value.map((item) => item.group_name || t('dashboard.noGroup')),
    datasets: [{
