<template>
  <div v-if="snapshot" class="space-y-1" data-testid="monitor-quota-view">
    ...
    <div v-if="snapshot.success && balanceRows.length" class="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px]">
      ...
      >
        {{ b.balance.toFixed(2) }} {{ currencyLabel(b.currency) }}
      </span>
    </div>
...
import type { MonitorQuotaSnapshot, MonitorQuotaTier } from '@/types/channelMonitor'
import MonitorQuotaProgress from '@/components/user/monitor/MonitorQuotaProgress.vue'

/** Render quota snapshots returned by the public channel-monitor API. */
...
function currencyLabel(currency: string): string {
  return currency.toUpperCase() === 'USD' ? '元' : currency
}
