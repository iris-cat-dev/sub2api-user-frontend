...
export async function getDimensions(filter: MonitorFilter, signal?: AbortSignal) {
  const { data } = await apiClient.get<MonitorDimensions>('/channel-monitor-v2/dimensions', requestConfig(filter, signal))
  return data
}

export async function getSnapshot(filter: MonitorFilter, signal?: AbortSignal) {
  const { data } = await apiClient.get<MonitorSnapshot>('/channel-monitor-v2/snapshot', requestConfig(filter, signal))
  return data
}

export async function getMatrix(filter: MonitorFilter, groupBy: MonitorMatrixGroupBy, signal?: AbortSignal) {
  const { data } = await apiClient.get<MonitorMatrixResponse>('/channel-monitor-v2/matrix', requestConfig(filter, signal, { group_by: groupBy }))
  return data
}

export async function getModels(filter: MonitorFilter, signal?: AbortSignal) {
  const { data } = await apiClient.get<{ coverage: MonitorCoverage; items: MonitorModelRow[] }>('/channel-monitor-v2/models', requestConfig(filter, signal))
  return data
}

export async function getErrors(filter: MonitorFilter, signal?: AbortSignal) {
  const { data } = await apiClient.get<{ coverage: MonitorCoverage; items: MonitorErrorRow[] }>('/channel-monitor-v2/errors', requestConfig(filter, signal))
  return data
}

export async function getUsers(filter: MonitorFilter, signal?: AbortSignal) {
  const { data } = await apiClient.get<{ coverage: MonitorCoverage; items: MonitorUserRow[] }>('/channel-monitor-v2/users', requestConfig(filter, signal))
  return data
}
